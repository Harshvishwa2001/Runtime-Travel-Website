...
export default function JourneyPath() {
    return (
        <section className="relative w-full min-h-screen overflow-hidden py-20 flex flex-col justify-center items-center">
            ...
            {/* Content Layer */}
            <div className="relative z-10 w-full text-center mx-auto px-6 md:px-12 lg:px-24 flex flex-col items-center justify-center">
                <p className='font-sans text-[16px] text-[#C8913A] uppercase font-medium tracking-widest'> Your Path</p>
                <h2 className='text-xl md:text-[56px] text-[#1A1A1A] mt-2'> From plains to peaks. </h2>
                <p className='font-secondary text-black text-[12px] md:text-[16px] leading-relaxed mt-2'>
                ...
                <div className='w-full max-w-190 bg-white/65 backdrop-blur-sm px-4 md:px-10 py-12 mt-12 rounded-2xl border border-[#C8913A33]'>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-y-12 gap-x-4 lg:hidden max-w-2xl mx-auto px-6">
                            {journeyRoute.map((node, index) => (
                                <div key={index} className="flex flex-col items-center text-center relative">
                                    <span className="text-xs md:text-sm tracking-[0.15em] text-[#1A1A1A] font-medium mb-3">
                                        {node.city}
                                    </span>
